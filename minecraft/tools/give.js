...
function give() {
...

  {
    ...
    if (target_x!=NaN) {selector.push('x=' + target_x);}
    if (target_y!=NaN) {selector.push('y=' + target_y);}
    if (target_z!=NaN) {selector.push('z=' + target_z);}

    if (selection_area == 'radius' && (dist_min!=NaN || dist_max!=NaN)) {
    ...
    if (selection_area == 'volume' && (vol_x!=NaN || vol_y!=NaN || vol_z!=NaN)) {
        if (vol_x!=NaN) {selector.push('dx=' + vol_x);}
        if (vol_y!=NaN) {selector.push('dy=' + vol_y);}
        if (vol_z!=NaN) {selector.push('dz=' + vol_z);}
    }

    if (xrot_min || xrot_max) {
        if (xrot_min!=NaN && !xrot_max!=NaN) {selector.push('x_rotation=' + xrot_min + '..');}
        if (!xrot_min!=NaN && xrot_max!=NaN) {selector.push('x_rotation=' + '..' + xrot_max);}
        if (xrot_min!=NaN && xrot_max!=NaN && xrot_min != xrot_max) {selector.push('x_rotation=' + xrot_min + '..' + xrot_max);}
        if (xrot_min!=NaN && xrot_max!=NaN && xrot_min == xrot_max) {selector.push('x_rotation=' + xrot_min);}
    }

    if (yrot_min!=NaN || yrot_max!=NaN) {
        if (yrot_min!=NaN && !yrot_max!=NaN) {selector.push('y_rotation=' + yrot_min + '..');}
        if (!yrot_min!=NaN && yrot_max!=NaN) {selector.push('y_rotation=' + '..' + yrot_max);}
        if (yrot_min!=NaN && yrot_max!=NaN && yrot_min != yrot_max) {selector.push('y_rotation=' + yrot_min + '..' + yrot_max);}
        if (yrot_min!=NaN && yrot_max!=NaN && yrot_min == yrot_max) {selector.push('y_rotation=' + yrot_min);}
    }

    if (limit!=NaN) {selector.push('limit=' + limit);}
    ...
    if (xp_min!=NaN || xp_max!=NaN) {
        if (xp_min!=NaN && !xp_max!=NaN) {selector.push('level=' + xp_min + '..');}
        if (!xp_min!=NaN && xp_max!=NaN) {selector.push('level=' + '..' + xp_max);}
        if (xp_min!=NaN && xp_max!=NaN && xp_min != xp_max) {selector.push('level=' + xp_min + '..' + xp_max);}
        if (xp_min!=NaN && xp_max!=NaN && xp_min == xp_max) {selector.push('level=' + xp_min);}
    }
    
    if (score && (score_min!=NaN || score_max!=NaN)) {
        scores.push(score);
        ...
        for (i = 0; i < scores.length; i++) {
            if (scores[i]) {
                if (scores_min[i]!=NaN && !scores_max[i]!=NaN) {score_text.push(scores[i] + '=' + scores_min[i] + '..');}
                if (!scores_min[i]!=NaN && scores_max[i]!=NaN) {score_text.push(scores[i] + '=' + '..' + scores_max[i]);}
                if (scores_min[i]!=NaN && scores_max[i]!=NaN && scores_min[i] != scores_max[i]) {score_text.push(scores[i] + '=' + scores_min[i] + '..' + scores_max[i]);}
                if (scores_min[i]!=NaN && scores_max[i]!=NaN && scores_min[i] == scores_max[i]) {score_text.push(scores[i] + '=' + scores_min[i]);}
            }
        }
