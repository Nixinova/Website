...
function give() {
    ...
    var target_x = value('input_selector_x').match(/^[~^]?-?[0-9]*\.?[0-9]+$/g, '');
    var target_y = value('input_selector_y').match(/^[~^]?-?[0-9]*\.?[0-9]+$/g, '');
    var target_z = value('input_selector_z').match(/^[~^]?-?[0-9]*\.?[0-9]+$/g, '');
    ...
    $('#generator-output').html(`
        ...
        <span style="color: #ff5">${item + NBT.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</span>
